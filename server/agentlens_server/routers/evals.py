...
def _runs_with_tag(rows, tag: str) -> list[dict]:
    return [
        {"run_id": r.run_id, "name": r.name, "status": r.status,
         "scores": r.scores or [], "tags": r.tags or []}
        for r in rows
        if tag in (r.tags or [])
    ]
...
async def judge(req: JudgeRequest, session: AsyncSession = Depends(get_session)):
    ...
    run = {
        ...
        "total_cost_usd": row.total_cost_usd, "error": row.error, "spans": row.spans or [],
    }
    try:
...
async def gate(req: GateRequest, session: AsyncSession = Depends(get_session)):
    ...
    rows = (await session.execute(
        select(RunRow).order_by(desc(RunRow.started_at)).limit(max(req.limit, 1) * 4)
    )).scalars().all()
